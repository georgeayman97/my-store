...
<div class="form-group">
        ...
        <!-- @ error ==> is laravel internal $errors var 
            also we have internal msg using $message instead of $errors->get('name')[0] -->
            @error('name')
    ...
    <div class="form-group">
        ...
        <input type="file" class="form-control @error('image') is-invalid @enderror" value="{{ old('image_path',$product->image_path)}}" name="image">
        @error('image')
        <p class="invalid-feedback">{{ $message }}</p>
        @enderror
    </div>
